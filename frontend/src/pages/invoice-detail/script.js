...
function addRow(args) {
  ...
  let row = `
    ...
    <td contenteditable=true class="table-column">${
      customerOrdersDB[0].job_number
    }</td>
    <td contenteditable=true class="table-column">${
      customerOrdersDB[0].order_notes
    }</td>
    <td contenteditable=true class="table-column">${
      customerOrdersDB[0].type
    }</td>
    <td contenteditable=true class="table-column">${
      customerOrdersDB[0].quantity
    }</td>
    <td contenteditable=true class="table-column">${
      customerOrdersDB[0].total_cost
    }</td>
    <td contenteditable=true class="table-column">${
      customerOrdersDB[0].total_cost * customerOrdersDB[0].quantity
    }</td>
  ...
  // Bind event listener to the select element
  table
    .find(".invoice-select")
    .last()
    .on("change", function () {
      const selectedOrderId = $(this).val();
      const selectedOrder = customerOrdersDB.find(
        (order) => order.order_id === selectedOrderId
      );

      if (selectedOrder) {
        const row = $(this).closest("tr");
        const cells = row.find("td").not(":first"); // Ignore the hidden and select cells
        cells[1].textContent = selectedOrder.job_number;
        cells[2].textContent = selectedOrder.order_notes;
        cells[3].textContent = selectedOrder.type;
        cells[4].textContent = selectedOrder.quantity;
        cells[5].textContent = selectedOrder.total_cost;
        cells[6].textContent =
          selectedOrder.total_cost * selectedOrder.quantity;
      }
    });
...
async function saveInvoiceItems(invoiceId, invoiceItems) {
  try {
    const response = await fetch(
      `${BASE_URL}/save-invoice-items/${invoiceId}/`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
function deleteInvoice() {
  showConfirmation(
    ...
    () => {
      ...
      fetch(`${BASE_URL}/invoice/${invoiceId}`, { method: "DELETE" })
        .then((response) => {
          if (!response.ok) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .getElementById("changePrinterButton")
